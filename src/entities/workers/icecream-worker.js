...
import { EmployeeState, StaffPriority } from "../../enum/employee-state";
...
export class IceCreamWorker {
    ...
    onIdle() {
        ...
        for (let i = 0; i < this.board.balconies.length; i++) {
            ...
            if (GameVars.game.management.staffPriority === StaffPriority.PATIENCE) {
    ...
    setTargetIceCreamMachine(index) {
        const iceCreamMachine = this.board.iceCreamMachine[this.customer.flavoursColors[index]];
    ...
    onReachIceCreamMachine() {
        ...
        const iceCreamMachine = this.board.iceCreamMachine[targetColor];
